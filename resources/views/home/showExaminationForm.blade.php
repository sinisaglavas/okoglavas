    ...
    <div class="container">
        <div class="row">
            <div class="col-4">
                ...
                <a href="{{ route('home.singleClient',['id'=>$single_client->id]) }}" class="btn btn-light form-control m-2">Client: {{ $single_client->name }}</a>
                ...
                <form action="{{ route('home.saveDistance',['id'=>$single_client->id]) }}" method="POST">
                    ...
                    <select name="right_eye_pd" class="form-control" id="right-eye-pd">
                        @foreach($all_pd as $pd)
                            <option name="right_eye_cylinder" value="{{ $pd->pd_range }}">{{ $pd->pd_range }}</option>
                        @endforeach
                    </select><br>
