        ...
        <div class="row">
            <div class="col-3">
                ...
                <div class="border border-opacity-10 m-3 p-3">
                    ...
                    <h6>Ukupno plaćeno: {{ \App\Models\PaymentContactLense::where('debtors_contact_lenses_id', $debtor->id)->sum('payment') }} dinara</h6>
                    @if($debtor->debit - \App\Models\PaymentContactLense::where('debtors_contact_lenses_id', $debtor->id)->sum('payment') > 0)
                        <h6>Trenutno zaduženje: <span style="color: #b82830; font-weight: bold">{{ $debtor->debit - \App\Models\PaymentContactLense::where('debtors_contact_lenses_id', $debtor->id)->sum('payment') }} dinara</span></h6>
                    @elseif($debtor->debit - \App\Models\PaymentContactLense::where('debtors_contact_lenses_id', $debtor->id)->sum('payment') <= 0)
                        <h6>Trenutno stanje: <span style="font-weight: bold">{{ $debtor->debit - \App\Models\PaymentContactLense::where('debtors_contact_lenses_id', $debtor->id)->sum('payment') }}</span></h6>
                    @endif
                </div>
            </div>
            <div class="col-1"></div>
